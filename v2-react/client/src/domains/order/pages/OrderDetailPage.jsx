                ...
                return (
                  <div key={step} className="flex flex-col items-center flex-1 relative">
                    ...
                    <div className={`relative z-10 flex h-7 w-7 items-center justify-center rounded-full border-2 ${
                      isCompleted ? 'border-primary-600 bg-primary-600' : 'border-gray-300 bg-white'
                    }`}>
                      {isCompleted ? (
          ...
          <div className="rounded-xl bg-white shadow-sm ring-1 ring-gray-200 p-6">
            ...
            <div className="space-y-2">
              {canPerformAction(user?.role, order.status, 'submit') && (
                <button onClick={() => handleAction(submitOrder, 'submitted')} disabled={actionLoading} className="btn-primary w-full">Submit Order</button>
              )}
              {canPerformAction(user?.role, order.status, 'prepare') && (
                <button onClick={() => handleAction(prepareOrder, 'moved to preparing')} disabled={actionLoading} className="btn-primary w-full">Start Preparing</button>
              )}
              {canPerformAction(user?.role, order.status, 'ship') && (
                <button onClick={() => handleAction(shipOrder, 'shipped')} disabled={actionLoading} className="btn-primary w-full">Mark as Shipped</button>
              )}
              {canPerformAction(user?.role, order.status, 'receive') && (
                <button onClick={() => handleAction(receiveOrder, 'received')} disabled={actionLoading} className="btn-success w-full">Confirm Received</button>
              )}
              {canPerformAction(user?.role, order.status, 'complete') && (
                <button onClick={() => handleAction(completeOrder, 'completed')} disabled={actionLoading} className="btn-success w-full">Complete Order</button>
              )}
              {canPerformAction(user?.role, order.status, 'edit') && (
                <Link to={`/orders/${order.id}/edit`} className="btn-secondary w-full text-center block">Edit Order</Link>
              )}
              {canPerformAction(user?.role, order.status, 'cancel') && (
                <button onClick={() => setCancelDialogOpen(true)} disabled={actionLoading} className="btn-danger w-full">Cancel Order</button>
              )}
            </div>
          </div>
        </div>
