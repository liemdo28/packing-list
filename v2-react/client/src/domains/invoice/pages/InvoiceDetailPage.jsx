...
export default function InvoiceDetailPage() {
  ...
  const fetchInvoice = () => {
    setLoading(true);
    getInvoice(id).then((res) => setInvoice(res.data.data)).catch(() => setError('Failed to load invoice')).finally(() => setLoading(false));
  };
  ...
  const handleReconcile = async () => {
    ...
    try { await reconcileInvoice(id); setReconcileOpen(false); setSuccess('Invoice reconciled successfully'); fetchInvoice(); }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <Link to="/invoices" className="inline-flex items-center text-sm text-gray-500 hover:text-gray-700 mb-4"><ArrowLeftIcon className="h-4 w-4 mr-1" /> Back to Invoices</Link>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            {invoice.status === 'pending' && (
              <Link to={`/invoices/${id}/edit`} className="btn-secondary text-sm"><PencilSquareIcon className="h-4 w-4 mr-1" /> Edit</Link>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 rounded-xl bg-white shadow-sm ring-1 ring-gray-200 p-6">
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            ...
            <tfoot className="bg-gray-50">
              <tr><td colSpan={3} className="px-4 py-2 text-right font-semibold">Total</td><td className="px-4 py-2 text-right font-bold text-primary-700">{formatCurrency(invoice.total_amount)}</td></tr>
            </tfoot>
          </table>
        </div>
      ...
      <ConfirmDialog open={reconcileOpen} onClose={() => setReconcileOpen(false)} onConfirm={handleReconcile} title="Reconcile Invoice" message="Are you sure you want to mark this invoice as reconciled? This action cannot be undone." confirmText="Reconcile" confirmColor="primary" loading={actionLoading} />
    </div>
  );
}