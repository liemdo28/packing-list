...
export default function NotificationListPage() {
  ...
  const clearFilters = () => {
    setFilters({ type: '', isRead: '', severity: '' });
    pagination.goToPage(1);
  };
    ...
    <div>
      ...
      {showFilters && (
        <div className="mb-6 p-4 bg-white rounded-xl shadow-sm ring-1 ring-gray-200">
          ...
          {(filters.type || filters.isRead || filters.severity) && (
        ...
        <div className="rounded-xl bg-white shadow-sm ring-1 ring-gray-200 divide-y divide-gray-100">
          {notifications.length === 0 ? (
            <div className="py-12 text-center">
              ...
              <p className="mt-3 text-sm text-gray-500">No notifications found</p>
            </div>
          ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        {notif.severity && (
                          <span className={`text-xs rounded-full px-2 py-0.5 border ${getSeverityBadge(notif.severity)}`}>
                            {notif.severity}
                          </span>
                        )}
                        {notif.type && (
                          <span className={`text-xs rounded-full px-2 py-0.5 ${getTypeBadge(notif.type)}`}>
                            {notif.type}
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-3 mt-1.5 text-xs text-gray-400">
                      {notif.source_store_name && (
                        <span>From: {notif.source_store_name}</span>
                      )}
                      {notif.order_number && (
                        <span>Order: {notif.order_number}</span>
                      )}
