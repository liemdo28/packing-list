...
export default function NotificationListPage() {
  ...
  const handleRead = async (notif) => {
    if (!notif.is_read) { await markAsRead(notif.id); refetch(); fetchNotifications(); }
    if (notif.reference_type === 'order' && notif.reference_id) navigate(`/orders/${notif.reference_id}`);
  };

  const handleMarkAllRead = async () => { await markAllAsRead(); refetch(); fetchNotifications(); };
      ...
      {loading ? <LoadingSpinner className="py-12" /> : (
        <div className="rounded-xl bg-white shadow-sm ring-1 ring-gray-200 divide-y divide-gray-100">
          {notifications.length === 0 ? (
            <div className="py-12 text-center"><BellIcon className="mx-auto h-12 w-12 text-gray-300" /><p className="mt-3 text-sm text-gray-500">No notifications yet</p></div>
          ) : (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`text-xs ml-2 flex-shrink-0 rounded-full px-2 py-0.5 ${notif.type === 'order' ? 'bg-blue-100 text-blue-700' : notif.type === 'invoice' ? 'bg-green-100 text-green-700' : notif.type === 'alert' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700'}`}>{notif.type}</span>
                    </div>
