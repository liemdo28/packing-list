...
export const B1_USER        = __ENV.B1_USER        || 'user_b1';
export const B1_PASSWORD    = __ENV.B1_PASSWORD    || 'password';
export const B2_USER        = __ENV.B2_USER        || 'user_b2';
export const B2_PASSWORD    = __ENV.B2_PASSWORD    || 'password';
export const B3_USER        = __ENV.B3_USER        || 'user_b3';
 ...
 * API accepts { username, password } and returns { data: { token, user } }.
...
export function login(username, password) {
  ...
  check(res, {
    [`login ok for ${username}`]: (r) => r.status === 200,
