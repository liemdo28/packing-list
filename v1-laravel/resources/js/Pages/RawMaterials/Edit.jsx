...
export default function RawMaterialsEdit({ material }) {
            ...
            <div className="max-w-2xl mx-auto space-y-6">
                ...
                <form onSubmit={handleSubmit} className="bg-[#1e1e2e] border border-gray-700/50 shadow-lg rounded-lg p-6 space-y-4">
                    <div>
                        ...
                        <input type="text" value={data.code} onChange={(e) => setData('code', e.target.value)} className={inputClass} required />
                    ...
                    <div>
                        ...
                        <input type="text" value={data.base_unit} onChange={(e) => setData('base_unit', e.target.value)} className={inputClass} required />
                        {errors.base_unit && <p className="mt-1 text-sm text-red-400">{errors.base_unit}</p>}
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-300 mb-1">Category</label>
                        <input type="text" value={data.category} onChange={(e) => setData('category', e.target.value)} className={inputClass} />
                        {errors.category && <p className="mt-1 text-sm text-red-400">{errors.category}</p>}
                    </div>
                    <div className="flex items-center">
                        <input id="active" type="checkbox" checked={data.active} onChange={(e) => setData('active', e.target.checked)} className="h-4 w-4 rounded border-gray-600 bg-gray-800 text-purple-600 focus:ring-purple-500" />
                    ...
                    <div className="flex justify-end gap-3 pt-4">
                        ...
                        <button type="submit" disabled={processing} className="rounded-md bg-purple-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-purple-500 disabled:opacity-50">{processing ? 'Saving...' : 'Update Material'}</button>
                    </div>
                </form>
            </div>
        </Layout>
